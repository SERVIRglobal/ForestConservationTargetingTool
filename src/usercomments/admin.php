 ...
 * Last Modified: Sat, 10 August 2013 08:38:00 GMT
 * ----------------------------------------------
 */
ob_start();

// First I'm going to delete all existing variables except those we may need
$keep = array('GLOBALS', '_SERVER', '_GET', '_POST', '_COOKIE', 'keep', 'key', 'val');  
...
$getVars = array('action', 'tbl', 'gbsession', 'uid', 'panel', 'section', 'ip', 'rid', 'id', 'resetBlock', 'entry');
...
else
{
  ...
  switch ($action)
  {
   ...
   case 'multi':
    $postVars = array('tbl', 'rid', 'multidelete', 'multiaccept', 'multiunaccept', 'entryarray', 'comarray');
    create_variable($postVars);
    $multimess = '';
    if (isset($comarray))
    {
   ...
   case 'save':
    if ($panel == 'general')
    {
      if($section == 'general')
      {
        ...
        $postVars = array('entries_per_page', 'lang', 'book_name', 'charset', 'show_ip', 'allow_html', 'allowed_tags', 'allow_search', 
                          'smilies', 'always_flash', 'agcode', 'allow_urlagcode', 'allow_emailagcode', 'allow_imgagcode', 
                          'allow_flashagcode', 'agcode_img_width', 'agcode_img_height', 'encrypt_email', 'base_url', 'laz_url', 
                          'permalinks', 'use_gravatar', 'included');
      ...
      {
        // This is the variables for this section
        $postVars = array('require_email', 'allow_loc', 'allow_url', 'allow_gender', 'allow_icq', 'allow_aim', 'allow_msn', 
                          'allow_yahoo', 'allow_skype', 'allow_private', 'hide_comments', 'allow_img', 'img_width', 'img_height', 
                          'max_img_size', 'thumbnail', 'thumb_min_fsize', 'disablecomments');
      ...
      {
        // This is the variables for this section
        $postVars = array('admin_mail', 'book_mail', 'always_bookemail', 'notify_private', 'notify_admin', 'notify_admin_com', 
        'notify_guest', 'html_email', 'notify_mes', 'mail_type', 'smtp_server', 'smtp_username', 'smtp_password', 'smtp_port', 'mailSSL');
      ...
      {
        // This is the variables for this section
        $postVars = array('count_blocks', 'post_time_min', 'post_time_max', 'check_headers', 'antibottest', 'bottestquestion', 
        'bottestanswer', 'need_pass', 'comment_pass', 'com_question', 'solve_media', 'captcha_noise', 'sfs_confidence', 
        'captcha_grid', 'captcha_grey', 'captcha_trans', 'captcha_width', 'captcha_height', 'max_url', 'require_checking', 
        'require_comchecking', 'min_text', 'max_text', 'max_word_len', 'use_regex', 'badwords1', 'badwords2', 'badwords3', 
        'flood_check', 'flood_timeout', 'banned_ip', 'banned_ips', 'honeypot');
        ...
        if (!get_magic_quotes_gpc())
        {
          $bottestquestion = addslashes($bottestquestion);
          $bottestanswer = addslashes($bottestanswer);
          $comment_pass = addslashes($comment_pass);
          $com_question = addslashes($com_question);
        }
        $sqlquery= "UPDATE ".LAZ_TABLE_PREFIX."_config set use_regex='$use_regex', post_time_max='$post_time_max', post_time_min='$post_time_min', captcha_trans='$captcha_trans', antibottest='$antibottest', bottestquestion='$bottestquestion', bottestanswer='$bottestanswer', count_blocks='$count_blocks', sfs_confidence='$sfs_confidence', ";
        ...
        $sqlquery.="flood_timeout='$flood_timeout', captcha_noise='$captcha_noise', captcha_grid='$captcha_grid', captcha_grey='$captcha_grey', max_url='$max_url', need_pass='$need_pass', comment_pass='$comment_pass', com_question='$com_question', captcha_width='$captcha_width', captcha_height='$captcha_height' WHERE (config_id = '1')";
        $gb_auth->query($sqlquery);
    ...
    elseif ($panel == 'style')
    {
      ...
      {
        // This is the variables for this section
        $postVars = array('pbgcolor', 'width', 'font_face', 'link_color', 'text_color', 'tb_font_1', 'tb_font_2', 
        'laz_top_font_color', 'top_link_color', 'laz_top_num_color', 'tb_hdr_color', 'tb_text', 'tb_bg_color', 'tb_color_1', 
        'tb_color_2', 'search_bg_color', 'search_font_color', 'input_error_color', 'errorbox_border_color', 
        ...
        $sqlquery .= "errorbox_border_color='$errorbox_border_color', errorbox_border_width='$errorbox_border_width', errorbox_border_style='$errorbox_border_style', errorbox_font_color='$errorbox_font_color', errorbox_back_color='$errorbox_back_color', external_css='$external_css' WHERE (config_id = '1')";
      }
      elseif($section == 'date')
      {
        // This is the variables for this section
    ...
    {
      // This is the variables for this section
      $postVars = array('NEWadmin_name', 'NEWadmin_pass', 'comfirm');
