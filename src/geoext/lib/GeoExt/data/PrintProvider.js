...
GeoExt.data.PrintProvider = Ext.extend(Ext.util.Observable, {
    ...
    encoders: {
        "layers": {
            ...
            "XYZ": function (layer) {  //added 11/15/2015 for Matt Hansen Layer, by Len, using code from https://github.com/shr3k/geoext/commit/cde94993c3a78da3f2385a51c8c966bbf44efc8c
				var enc = this.encoders.layers.TileCache.call(this, layer);
				...
				resolutions: layer.serverResolutions || layer.resolutions,
					...
					//maptype: 'roadmap',
					format : 'png',
					layer: "hansenXYZproxy.php?url=",//app.mapPanel.map.getLayer(layer.id).name,
					baseURL: "http://conservationroi.net/scripts/hansenXYZproxy",//enc.baseURL.substr(0, enc.baseURL.indexOf("$")),
					extension: enc.baseURL.substr(enc.baseURL.lastIndexOf("$")).split(".").pop(),
					//tileOriginCorner: layer.tileOriginCorner
				});
								//alert(layer.extension);
				//alert(enc.baseURL);
			/*var maxExtent = layer.maxExtent.toArray();
            var tileOriginX = layer.tileOrigin ? layer.tileOrigin.lon : maxExtent[0];
            var tileOriginY = layer.tileOrigin ? layer.tileOrigin.lat : maxExtent[1];
            return Ext.apply(enc, {
                ...
                tileOrigin: {x: tileOriginX, y: tileOriginY},
                tileSize: [layer.tileSize.w, layer.tileSize.h],
                layer: app.mapPanel.map.getLayer(layer.id).name,
					//baseURL: enc.baseURL.substr(0, enc.baseURL.indexOf("$")),
				extension: enc.baseURL.substr(enc.baseURL.lastIndexOf("$")).split(".").pop(),
                resolutions: layer.serverResolutions || layer.resolutions
            });
        ...
        "legends": {
            "gx_wmslegend": function(legend) {
				...
				//Had to add this next line to pass layer name through and prevent an error from the name not being defined. Not sure why it wasn't there before... LTG, 11/12/15
				enc[0].name = app.mapPanel.map.getLayer(legend.layerRecord.id).name;
                enc[0].classes[0] = {
                    name: "",
                    icons: icons
                };
